...
#Region "开始"
    ...
    ''' 校验档案是否有效
    ''' </summary>
    ''' <returns>错误信息，为空则表示校验通过</returns>
    Private Function IsProfileValid() As String
        If SelectedProfile Is Nothing Then
            Return "请先选择一个档案！"
        End If
        Select Case SelectedProfile.Type
            Case McLoginType.Legacy
                If String.IsNullOrWhiteSpace(SelectedProfile.Username) Then
                    Return "离线档案用户名不能为空！"
                End If
            Case McLoginType.Auth
                If String.IsNullOrWhiteSpace(SelectedProfile.Name) OrElse String.IsNullOrWhiteSpace(SelectedProfile.Password) Then
                    Return "第三方验证档案用户名或密码不能为空！"
                End If
                If String.IsNullOrWhiteSpace(SelectedProfile.Server) Then
                    Return "第三方验证档案服务器地址不能为空！"
                End If
        End Select
        Return ""
    End Function
    ...
    Private Function GetLoginData() As McLoginData
        If SelectedProfile Is Nothing Then
            Throw New Exception("未选择档案！")
        End If
        Select Case SelectedProfile.Type
            Case McLoginType.Legacy
                Return New McLoginLegacy With {
                    .UserName = SelectedProfile.Username,
                    .Uuid = SelectedProfile.Uuid
                }
            Case McLoginType.Auth
                Return New McLoginServer With {
                    .UserName = SelectedProfile.Name,
                    .Password = SelectedProfile.Password,
                    .BaseUrl = SelectedProfile.Server,
                    .Description = SelectedProfile.ServerName
                }
            Case Else
                Throw New Exception("不支持的登录类型：" & SelectedProfile.Type.ToString())
        End Select
    End Function
...
#End Region
    ...
    Private Sub McLoginStart(Data As LoaderTask(Of McLoginData, McLoginResult))
        ...
        Select Case Data.Input.Type
            ...
            Case Else
                Throw New Exception("不支持的登录类型：" & Data.Input.Type.ToString())
        End Select
        '尝试加载
